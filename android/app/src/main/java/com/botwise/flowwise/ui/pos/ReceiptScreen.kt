...
import com.botwise.flowwise.data.printing.ReceiptPrinter
import com.botwise.flowwise.data.printing.buildEscPosReceipt
...
fun ReceiptScreen(
    ...
    // Phase 6: Bluetooth thermal printing (best-effort — never blocks the sale).
    val context = LocalContext.current
    val printer = remember { ReceiptPrinter() }
    ...
    val bluetoothPermissionLauncher = rememberLauncherForActivityResult(
        ...
        if (granted) showPrinterPicker = true else printError = "Bluetooth permission denied — enable it to print"
    }
    val openPrinterPicker: () -> Unit = {
    ...
    Column(
        ...
        OutlinedButton(
            ...
            Text(if (printing) "Printing…" else "Print receipt (Bluetooth)")
        }
    ...
    if (showPrinterPicker) {
        val devices = printer.bondedPrinters()
        AlertDialog(
            ...
            text = {
                if (devices.isEmpty()) {
                    Text(
                        "No bonded Bluetooth printers. Pair one in Android Settings, then try again.",
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
                } else {
                    LazyColumn {
                                ...
                                Modifier.fillMaxWidth()
                                    .clickable {
                                        ...
                                        scope.launch {
                                            ...
                                            printError = null
                                            try {
                                                printer.print(device, buildEscPosReceipt(receipt))
                                            } catch (e: Exception) {
                                                printError = e.message ?: "Could not print — check the printer is on and paired"
                                            } finally {
                                                printing = false
                                            }
                                        }
